...
export class StrategyEngine {
  ...
  private async runStage2(isManual: boolean = false): Promise<boolean> {
    ...
    if (passedResults.length > 0 && !isManual && this.isRunning && !this.isOrdering && !this.pendingOrderSymbol) {
      if (Date.now() - startTime <= (this.settings.order?.positiveWindow || 2) * 1000) {
        // 过滤掉已经持有的币对、正在挂单的币对以及锁定的币对
        const eligibleResults = passedResults.filter(r => {
        ...
        if (remainingCapacity > 0 && eligibleResults.length > 0) {
          ...
          this.addLog('下单', `检测到当前持仓数量: ${currentActiveCount}, 最大限制: ${maxAllowed}, 将对前 ${tradeCount} 个符合条件的币对下单`, 'info');
  ...
  private async handleTPAndSL(symbol: string, quantity: string, scanStartTime: number, symbolInfo: any, orderId: string, executionData?: { avgPrice: number, qty: number }, side: 'BUY' | 'SELL' = 'BUY', scannerData?: any, isSecondOrder: boolean = false) {
    ...
    try {
      ...
      if (tpMode === 'fixed') {
        const defaultTpFixed = side === 'BUY' ? this.settings.order.tpFixedBuy : this.settings.order.tpFixedSell;
        const tpFixed = isSecondOrder
        ...
        const defaultTpAmp = side === 'BUY' ? this.settings.order.tpAmpBuy : this.settings.order.tpAmpSell;
        const tpAmp = isSecondOrder
        ...
        const defaultTpRatio = side === 'BUY' ? this.settings.order.tpRatioBuy : this.settings.order.tpRatioSell;
        const tpRatioVal = isSecondOrder
      ...
      if (slMode === 'fixed') {
        const defaultSlFixed = side === 'BUY' ? this.settings.order.slFixedBuy : this.settings.order.slFixedSell;
        const slFixed = isSecondOrder
        ...
        const amp = amplitudeMode === 'highLow' && kBestLow > 0
          ? (kBestHigh / kBestLow - 1)
          : (kBestHigh > 0 ? (1 - kBestLow / kBestHigh) : 0);
        const defaultSlAmp = side === 'BUY' ? this.settings.order.slAmpBuy : this.settings.order.slAmpSell;
        const slAmp = isSecondOrder
        ...
        const defaultSlRatio = side === 'BUY' ? this.settings.order.slRatioBuy : this.settings.order.slRatioSell;
        const slRatioVal = isSecondOrder
